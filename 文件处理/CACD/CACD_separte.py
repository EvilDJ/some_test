...
train_ = r'F:/网络数据集/cacd数据集/train_'
...
for dir_a, dir_n, file_n in os.walk(path_):#300
    ...
    if len(file_n) >= 40:
        #写匹配数据集的文件
        num = random.sample(range(len(file_n)), 40)
        # 在file_n的范围内，生成4个不相等的数字
        # num_m = len(num)/2
        for u in range(20):
            img1 = Image.open(dir_a + '/' + file_n[(num[u])])
            img1.save(train_l + '/' + 'pair_l_' + str(nl) + '.jpg')
            # img2 = Image.open(dir_a + '/' + file_n[(num[u+20])])
            # img2.save(train_r + '/' + 'pair_r_' + str(nl) + '.jpg')
            img3 = Image.open(dir_a + '/' + file_n[(num[u + 20])])
            img3.save(train_ + '/' + 'pair_' + str(nl) + '.jpg')
            print(nl, '\t', file_n[(num[u])], file_n[(num[u+20])], '写入到label标签文件下', '0')
            nl += 1
...
for j in range(len(dir_name)):
    if j!=299:
        for x in range(20):
            ...
            img1_r = Image.open(dir_name[j + 1][x+20])
    ...
    if j == 299:
        for x in range(20):
            img1_l = Image.open(dir_name[j][x])
            img1_l.save(train_l + '/' + 'pairl_' + str(tl) + '.jpg')
            img1_r = Image.open(dir_name[0][x+20])
            img1_r.save(train_r + '/' + 'pairr_' + str(tl) + '.jpg')
            te_p1.write('1' + '\n')
            print(tl, '\t', dir_name[j][x], dir_name[0][x+20], '写入到label标签文件下', '1')
            tl += 1
